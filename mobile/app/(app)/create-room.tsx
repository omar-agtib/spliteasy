  ...
  return (
    <View className="flex-1 bg-white px-5 pt-14">
      <Animated.View entering={FadeInDown.duration(450)}>
        <Text className="text-2xl font-semibold">{t("createRoom.title")}</Text>
        <Text className="text-zinc-500 mt-1">{t("createRoom.subtitle")}</Text>
      </Animated.View>
      ...
      >
        <Card>
          <Text className="text-zinc-700 mb-2">{t("createRoom.name")}</Text>
          <TextInput
            value={name}
          ...
          <PrimaryButton title={t("createRoom.cta")} onPress={create} />
          <PrimaryButton
            title={t("common.back")}
            onPress={() => router.back()}
            variant="light"
